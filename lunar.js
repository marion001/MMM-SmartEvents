...
if (typeof window.Lunar === "undefined") {
    ...
    function getYearInfo_smart_event(year) {
        if (year < 2100) return decodeLunarYear_smart_event(year, TK21_Lunar[year - 2000]);
        return decodeLunarYear_smart_event(year, TK22_Lunar[year - 2100]);
    }

    function lunarToSolar_smart_event(day, month, year) {
        let ly = getYearInfo_smart_event(year);
        for (let i = 0; i < ly.length; i++) {
            let m = ly[i];
            if (m.month === month && m.leap === 0) {
                let jd = m.jd + (day - 1);
                return jdToDate_smart_event(jd);
            }
        }
        return null;
    }

    function solarToLunar_smart_event(date) {
        let jd = jdn_smart_event(date.getDate(), date.getMonth() + 1, date.getFullYear());
    ...
    window.Lunar = {
        toSolar: function(dateStr, year) {
            ...
            return lunarToSolar_smart_event(d, m, year);
        },
        fromDate: solarToLunar_smart_event
    };
